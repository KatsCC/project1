...
import { FreindData } from "./ListCom";

export default function FriendCom({ friend }: { friend: FreindData[] }) {
  const [selectedFriend, setSelectedFriend] = useState("");
  const [selectedFriendId, setSelectedFriendId] = useState("");

  return (
    <ul>
      {friend.map((val: FreindData, index: number) => (
        <li
          key={index}
          onClick={() => {
        ...
        >
          <div className="flex items-center mb-2 pt-2 pl-4 border-t border-gray-200 cursor-pointer">
            <img
              src={val.image}
              alt="profile"
